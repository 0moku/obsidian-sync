...
9. **ANTHROPIC_API_KEY 확인/설정**
   - `~/.claude/settings.json`의 `env` 섹션에 `ANTHROPIC_API_KEY`가 있는지 확인
   - 없으면 아래 순서로 자동 탐지:
     1. 현재 환경변수 `$ANTHROPIC_API_KEY`
     2. 프로젝트 `.env` 파일
     3. `server/.env` 파일
   - 탐지되면 `~/.claude/settings.json`의 `env`에 자동 추가
   - 탐지 안 되면 사용자에게 입력 요청
   - 이미 설정되어 있으면 스킵
...
2. vault에서 해당 프로젝트의 `status.md` 읽기
